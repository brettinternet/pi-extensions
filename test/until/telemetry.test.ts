...
import {
  createTelemetrySink,
  hashCondition,
  TELEMETRY_ROTATION_BYTES,
  parseTelemetryLines,
  readTelemetry,
  summarizeTelemetry,
  summaryText,
  telemetryOptionsFromEnv,
} from "../../extensions/until/telemetry.ts";
...
describe("telemetry", () => {
  ...
  it("rotates bounded telemetry files and keeps one backup", async () => {
    const directory = mkdtempSync(join(tmpdir(), "pi-until-telemetry-"));
    tempDirectories.push(directory);
    const filePath = join(directory, "events.jsonl");
    writeFileSync(filePath, "x".repeat(TELEMETRY_ROTATION_BYTES + 1), "utf8");
    const sink = createTelemetrySink({ enabled: true, filePath });
    await sink.record("s", { count: 1, event: "suspended" });
    expect(readFileSync(`${filePath}.1`, "utf8")).toHaveLength(
      TELEMETRY_ROTATION_BYTES + 1,
    );
    expect(readFileSync(filePath, "utf8")).toContain('"event":"suspended"');
  });

  it("replaces the previous rotation backup", async () => {
    const directory = mkdtempSync(join(tmpdir(), "pi-until-telemetry-"));
    tempDirectories.push(directory);
    const filePath = join(directory, "events.jsonl");
    writeFileSync(filePath, "x".repeat(TELEMETRY_ROTATION_BYTES + 1), "utf8");
    writeFileSync(`${filePath}.1`, "old backup", "utf8");
  ...
  it("retries directory creation after a transient failure", async () => {
    ...
    const filePath = join(blockedParent, "events.jsonl");
    writeFileSync(blockedParent, "not a directory", "utf8");
    const sink = createTelemetrySink({ enabled: true, filePath });

    await sink.record("s", { count: 1, event: "suspended" });
    unlinkSync(blockedParent);
    await sink.record("s", { count: 2, event: "resumed" });

    expect(readFileSync(filePath, "utf8")).toContain('"event":"resumed"');
  });
  ...
  it("does nothing when disabled and reads env for options", async () => {
    ...
    expect(
      telemetryOptionsFromEnv({ PI_UNTIL_TELEMETRY: "1", PI_UNTIL_TELEMETRY_FILE: " /x/y.jsonl " })
    ).toMatchObject({
      enabled: true,
      filePath: "/x/y.jsonl",
    });

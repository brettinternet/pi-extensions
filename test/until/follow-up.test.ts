...
} from "../../extensions/until/follow-up.ts";
...
describe("session follow-up queue", () => {
  ...
  it("dispatches queued work restored after reload", () => {
    const dispatch = vi.fn();
    ...
    const actor = createActor(
      ...
      {
        input: {
          restored: { phase: "busy", queue: [queued] },
          sessionBusy: false,
        },
      }
    );
  ...
  it("waits for acknowledgement of an already-dispatched restored message", () => {
    const dispatch = vi.fn();
    const started = vi.fn();
    const active = { ...request("active"), dispatchedAt: 42 };
    const actor = createActor(
      createFollowUpMachine({
        dispatch,
        settled: () => {},
      ...
      {
        input: {
          restored: { active, phase: "awaitingStart", queue: [] },
          sessionBusy: false,
        },
      }
    );
    ...
    actor.start();
    expect(dispatch).not.toHaveBeenCalled();
    expect(actor.getSnapshot().matches("restoredAwaitingStart")).toBe(true);

    actor.send({ id: active.id, type: "MESSAGE_STARTED" });
    expect(started).toHaveBeenCalledWith(active);
    expect(actor.getSnapshot().matches("awaitingSettlement")).toBe(true);
  });

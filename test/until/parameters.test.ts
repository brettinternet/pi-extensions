...
import {
  ...
  untilParameters,
} from "../../extensions/until/command.ts";
...
describe("until tool parameters", () => {
  ...
  it("defaults one-shot watches to 24 hours and caps explicit timeout at 30 days", () => {
    const command = parseUntilCommand(
      { action: "start", condition: "true" },
      context,
    );
    expect(command.action).toBe("start");
    if (command.action !== "start") throw new Error("expected start command");
    expect(command.definition.expiresAt).toBe(context.startedAt + 86_400_000);
    expect(() =>
      parseUntilCommand(
        { action: "start", condition: "true", timeoutSeconds: 2_592_001 },
        context,
      ),

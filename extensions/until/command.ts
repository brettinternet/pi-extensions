...
const DEFAULT_UNTIL_TIMEOUT_SECONDS = 86_400;
const MAX_TIMEOUT_SECONDS = 2_592_000;
...
export const untilParameters = Type.Object(
  {
    ...
    label: Type.Optional(
      Type.String({
        maxLength: 120,
        description: "Short safe label. Do not include secrets.",
      })
    ),
    ...
    timeoutSeconds: Type.Optional(
      Type.Number({
        ...
        description:
          "Overall watch lifetime in seconds. One-shot defaults to 24 hours; recurring is required. Maximum is 30 days.",
      })
    ),
...
const startCommandFrom = (
  ...
  const definition: UntilDefinition = {
    ...
    expiresAt:
      context.startedAt +
      (params.timeoutSeconds ?? DEFAULT_UNTIL_TIMEOUT_SECONDS) * 1_000,
  };

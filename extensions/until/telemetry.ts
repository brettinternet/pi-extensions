...
export const TELEMETRY_ROTATION_BYTES = 5 * 1024 * 1024;
...
export const telemetryOptionsFromEnv = (
  ...
  enabled: env.PI_UNTIL_TELEMETRY === "1",
...
export const createTelemetrySink = (
  ...
  const writeOne = async (sessionId: string, event: TelemetryEventInput) => {
    ...
    try {
      ...
      // Reset a rejected mkdir so a later event can retry directory creation.
  ...
  const record: TelemetrySink["record"] = async (sessionId, event) => {
    if (!enabled) return;
    const write = writes.then(() => writeOne(sessionId, event));

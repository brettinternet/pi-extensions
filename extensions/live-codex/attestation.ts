...
function cborHeader(major: number, value: number): Buffer {
  if (!Number.isSafeInteger(value) || value < 0) {
    throw new Error(`Invalid CBOR length: ${value}`);
  }
...
function attestationSignals(): Buffer {
  ...
  const languages = Buffer.concat([cborHeader(128, 1), cborText(locale)]);
  return cborMap([
    [cborUnsigned(0), cborUnsigned(1)],
    [cborUnsigned(1), languages],
...
function buildClientAttestation(result: DeviceCheckResult): string {
  ...
  if (result.supported && result.tokenBase64) {
    ...
    entries.push([
      cborText("error_code"),
      cborUnsigned(result.supported ? 4 : 3),
    ]);
  }
  ...
  entries.push([
    cborText("f"),
    Buffer.concat([cborHeader(64, signals.length), signals]),
  ]);

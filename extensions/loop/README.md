...
/loop +<count>            Add future iterations while active
/loop -<count>            Remove future iterations while active
...
A retune changes only the next boundary's future budget; it never changes the prompt. A subtraction may reduce that budget to zero, ending the loop after the active iteration. Invalid or ambiguous forms are rejected instead of guessing.
